...
fn build_same_chain(
    ...
    out.push(concat!(
        "With a handle derived per venue --- `zkpi::handles`, one seed and an unrelated point ",
...
fn build_vetting(out: &mut Document, vetting: Option<&Value>) -> HarnessResult<()> {
    ...
    out.push(concat!(
        ...
        "`rust/defmi/src/vetting.rs` is where that is recorded.\n"
    ));
    out.push(concat!(
