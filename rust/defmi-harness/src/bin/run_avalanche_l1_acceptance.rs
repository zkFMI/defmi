...
use defmi::avalanche::{AvalancheClient, AvalancheRpcClient, FacilityAvalancheBridge};
use defmi::facility::{
    AccountOpening, AssetDefinition, AssetKind, DefmiFacility, QuorumApproval, QuorumAuthorizer,
    SettlementOrder, StateLeg,
};
use defmi_harness::{next_value, HarnessResult};
...
fn parse_args() -> HarnessResult<Options> {
    let mut options = Options {
        ...
        out: defmi_harness::repo_root().join("artifacts/avalanche_l1_acceptance.json"),
        runner: None,
        avalanchego: None,
