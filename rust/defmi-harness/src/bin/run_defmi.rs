...
use defmi::assets::{AssetRegistry, BlindedTag};
use defmi::credit::{CreditCtx, Tranche, Waterfall};
use defmi::ledger::Ledger;
use defmi::netting::{
    prove_cash_reference, BatchAttestation, Cycle, Mode as NetMode, Order, PositionBook,
};
use defmi::note_settlement::{build_note_package, LegInput, NoteDefmi};
...
use defmi_harness::{unique_temp_dir, write_pretty_json, HarnessResult};
use zkfmi_measure::Summary;
use zkfmi_zk::bitrange::{prove_bounded, verify_bounded, BoundedProof};
use zkfmi_zk::pedersen::Pedersen;
use zkfmi_zk::sigma::{prove_product, verify_product, ProductProof};
use zkpi::{deal_quorum, frost, Bounds, Instruction, Issuer, Openings, Venue};
use rand::rngs::OsRng;
...
fn run_main() -> HarnessResult<()> {
    ...
    result.insert("rustc".into(), json!(defmi_harness::rustc_version()));
