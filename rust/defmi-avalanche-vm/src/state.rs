...
use defmi::application_reservation::{ApplicationReservationBinding, ApplicationReserveScope};
use defmi::central_bank_liquidity::BojLiquidityBook;
use defmi::cross_domain::{
    Committee as CrossDomainCommittee, CrossDomainBook, Domain as CrossDomain,
};
use defmi::facility::{QuorumAuthorizer, ZERO};
...
pub struct ApplicationReservationRecord {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub remaining_opening: Option<defmi::application_settlement::ApplicationOpening>,
}
...
pub struct NoteClaimRecord {
    ...
    pub authorization: defmi::note_chain::ClaimAuthorizationCommitment,
...
impl State {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (key, record) in &self.settlement_verifiers {
            let config = defmi::settlement_verifier::SettlementVerifierConfig {
...
mod tests {
    ...
    fn state_root_commits_boj_and_participant_registry_books() {
        ...
        use defmi::participant::RegistryConfiguration;
