...
use defmi::reconcile::{check, prove, Attestation};
use defmi::viewing::*;
...
fn spend_one(
    ...
    owner: &ScopedWallet,
    scope: &str,
    registry: &defmi::assets::AssetRegistry,
    rng: &mut OsRng,
) -> Scalar {
    use defmi::notes::{ring_for, Wallet};
    let key = asset_key();
...
fn a_wallet_can_say_what_it_spent_and_sign_it() {
    ...
    let owner = ScopedWallet::new(&mut rng);
    fill(&mut ledger, &owner, &["s"], 4);
    let registry = defmi::assets::AssetRegistry::new(Pedersen::new(b"qomm:defmi:note:v1"), 16);
