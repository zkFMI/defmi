...
fn keys() -> BTreeMap<String, defmi::governance::GovernanceSigner> {
    (0..7)
        .map(|index| {
            (
                format!("node-{index}"),
...
struct CreditFixture {
    ...
    facility: DefmiFacility,
    authorizer: QuorumAuthorizer,
    nodes: BTreeMap<String, defmi::governance::GovernanceSigner>,
...
fn quorum_is_bound_to_unique_keys_domain_and_before_root() {
    let repeated =
        defmi::governance::GovernanceSigner::generate("node-a", 0, i64::MAX as u64).unwrap();
...
fn one_guarantor_cannot_split_an_entity_cap_across_two_facility_ids() {
    ...
    duplicate.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
...
fn reducing_a_facility_below_existing_usage_freezes_without_erasing_exposure() {
    ...
    .unwrap();
    reduction.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
    ...
    premature_activation.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
    ...
    .unwrap();
    rehabilitation.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
    ...
    activate.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
...
fn frozen_facility_refuses_new_rfqs_but_existing_hold_can_expire() {
    ...
    freeze.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
...
fn expired_product_reservation_restores_asset_and_credit_atomically_without_owner_signature() {
    ...
    let reserve_link = defmi::asset_link::prove(
        &issuer.key,
        cash.asset_id,
    ...
    let release_link = defmi::asset_link::prove(
        &issuer.key,
        cash.asset_id,
...
fn product_settlement_for(kind: GuarantorKind) {
    ...
    maker_grant.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
    ...
    taker_grant.guarantor_signature = defmi::facility::sign_guarantor_message(
        &fixture.guarantor,
        &zkfmi_crypto::test_support::entity_pq_signer(&fixture.guarantor.to_bytes()),
    ...
    let verifier = defmi::settlement_verifier::SettlementVerifierConfig {
        venue_id,
        defmi_id,
        epoch: 9,
    ...
    let maker_reserve_link = defmi::asset_link::prove(
        &issuer.key,
        cash_asset.asset_id,
    ...
    let taker_reserve_link = defmi::asset_link::prove(
        &issuer.key,
        security.asset_id,
...
fn guarantor_control_requires_both_components_and_rejects_wrong_purpose_without_mutation() {
    ...
    control.guarantor_signature =
        defmi::facility::sign_guarantor_message(&fixture.guarantor, &pq, &body).unwrap();
    let approval = approve(
        &fixture.facility,
        &fixture.authorizer,
        &fixture.nodes,
