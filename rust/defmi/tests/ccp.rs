...
use defmi::ccp::*;
use defmi::credit::{CreditCtx, Tranche};
use zkfmi_zk::pedersen::{encode, Pedersen};
use rand::rngs::OsRng;
use rand::Rng;
...
fn key() -> Pedersen {
    Pedersen::new(b"qomm:defmi:v1").with_value_generator(zkfmi_zk::pedersen::asset_tag(7))
}
...
fn margined(ctx: &CreditCtx, handle: &[u8]) -> defmi::credit::CreditLine {
    let mut rng = OsRng;
    ctx.grant(
        handle,
        "cash",
...
fn each_house_resolves_its_own_default_in_its_own_waterfall() {
    ...
    let tranches: Vec<Tranche> = amounts
        ...
        .collect();
    let waterfall = defmi::credit::Waterfall::new(base.clone(), tranches, 64);
