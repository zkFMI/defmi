...
use defmi::assets::AssetRegistry;
use defmi::note_settlement::*;
use defmi::notes::{ring_for, NoteLedger, Wallet};
use zkfmi_measure::{hosts, time_ms};
use zkfmi_zk::pedersen::Pedersen;
use zkpi::{deal_quorum, frost, Bounds, Instruction, Issuer, Venue};
use rand::rngs::OsRng;
...
fn weight(p: &NoteDvpPackage) -> usize {
    let leg = |l: &NoteLeg| {
        ...
        4 * l.ring.len()
            + defmi::notes::encode_spend_proof(&l.spend)
                .unwrap()
                .len()
            + 32 * 5 * l.notes.len()
    };
    zkpi::wire::encode(&p.instruction).len() + leg(&p.securities) + leg(&p.cash) + 32 * 6
}
